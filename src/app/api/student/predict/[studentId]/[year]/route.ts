...
export async function GET(_request: Request, { params }: { params: Promise<{ studentId: string; year: string }> }) {
  ...
  try {
    const response = await fetch(`${STUDENT_API_BASE_URL}/students/predict/${studentId}/${year}`, { headers: { 'ngrok-skip-browser-warning': 'true', Accept: 'application/json' }, cache: 'no-store', signal: AbortSignal.timeout(15000) });
