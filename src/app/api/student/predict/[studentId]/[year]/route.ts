...
export async function GET(_request: Request, { params }: { params: Promise<{ studentId: string; year: string }> }) {
  ...
  try {
    const response = await fetch(`${STUDENT_API_BASE_URL}/students/students/${studentId}/predict/${year}`, { headers: { Accept: 'application/json', 'ngrok-skip-browser-warning': 'true' }, cache: 'no-store', signal: AbortSignal.timeout(15000) });
