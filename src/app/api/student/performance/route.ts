...
export async function GET() {
  try {
    const response = await fetch(`${STUDENT_API_BASE_URL}/students/1/performance`, { headers: { 'ngrok-skip-browser-warning': 'true', Accept: 'application/json' }, cache: 'no-store', signal: AbortSignal.timeout(15000) });
