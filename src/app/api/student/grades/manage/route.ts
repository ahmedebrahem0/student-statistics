...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${STUDENT_API_BASE_URL}/students/grades/manage`, { method: 'POST', headers: { 'ngrok-skip-browser-warning': 'true', Accept: 'application/json', 'Content-Type': 'application/json' }, body, cache: 'no-store', signal: AbortSignal.timeout(15000) });
