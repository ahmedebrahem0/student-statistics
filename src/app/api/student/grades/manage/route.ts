...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${STUDENT_API_BASE_URL}/students/grades/manage`, { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json', 'ngrok-skip-browser-warning': 'true' }, body, cache: 'no-store', signal: AbortSignal.timeout(15000) });
