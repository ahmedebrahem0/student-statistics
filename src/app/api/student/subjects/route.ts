...
export async function GET() {
  try {
    const response = await fetch(`${STUDENT_API_BASE_URL}/students/subjects`, { headers: { Accept: 'application/json', 'ngrok-skip-browser-warning': 'true' }, cache: 'no-store', signal: AbortSignal.timeout(15000) });
