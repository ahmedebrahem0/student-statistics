...
export default function PredictionPanel({ studentId }: { studentId: number }) {
  ...
  const { register, handleSubmit, setError, formState: { errors } } = useForm<FormValues>({ defaultValues: { year: '' } });
  ...
  return <section className="chart-panel panel prediction-panel" aria-label="توقع الأداء الدراسي">
    <div className="section-heading"><h2><Sparkles size={16} />توقع الأداء الدراسي</h2><span className="chart-subtitle">النتائج المعروضة صادرة من الباك إند</span></div>
    <form className="prediction-form" onSubmit={handleSubmit(submit)} noValidate><label>السنة الدراسية المستهدفة<input {...register('year')} type="number" inputMode="numeric" dir="ltr" placeholder="2028" aria-invalid={Boolean(errors.year)} />{errors.year && <span className="field-error">{errors.year.message}</span>}</label><button className="primary-button" type="submit" disabled={isFetching}>{isFetching ? 'جارٍ التوقع...' : 'توقّع الأداء'}</button></form>
